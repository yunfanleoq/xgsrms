<template>
  <ScrollContainer>
    <div ref="wrapperRef" class="user-account-setting" :class="[prefixCls]">
      <Tabs tab-position="left" :tabBarStyle="tabBarStyle" @tab-click="componentClick" v-model:activeKey="activeKey">
        <template v-for="item in componentList" :key="item.key">
          <TabPane>
            ...
            <slot name="component" v-if="activeKey === item.key && item.isSlot"></slot>
          </TabPane>
        </template>
      </Tabs>
    </div>
