<template>
  <BasicModal ref="modalRef" destroyOnClose wrapClassName="j-cgform-tab-modal" v-bind="$attrs" @register="registerModal" :width="896" @ok="handleSubmit">
    <!-- 标题区域 -->
     <template #title>
        <div class="titleArea">
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (!unref(isUpdate)) {
            ...
            console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&",data.jobDetail)
             requestSubTableData(xgsResumeWorksList, {id:data?.jobDetail?.id}, xgsResumeWorksTable)
             requestSubTableData(xgsResumeEdusList, {id:data?.jobDetail?.id}, xgsResumeEdusTable)
             requestSubTableData(xgsResumeHomeList, {id:data?.jobDetail?.id}, xgsResumeHomeTable)
        }
        // 隐藏底部时禁用整个表单
