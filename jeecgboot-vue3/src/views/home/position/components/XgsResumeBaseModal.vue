<template>
  <BasicModal
    ref="modalRef"
    destroyOnClose
    wrapClassName="j-cgform-tab-modal xgsResumeBaseModal"
    v-bind="$attrs"
    ...
    <div class="contentArea">
      ...
      <BasicForm @register="registerForm" ref="formRef" v-show="activeKey == refKeys[0]" name="XgsResumeBaseForm">
        <template #uploadResume="{ model, field }">
          <a-form-item label="文件路径" id="XgsUserResumeFileForm-filePath" name="filePath">
            <j-upload v-model:value="formData.filePath" :max-count="1" :multiple="false" />
            <a-button type="primary" :disabled="!formData.filePath" @click="analysisResume">简历分析</a-button>
...
import {ref, computed, unref, reactive, toRefs, toRef} from 'vue';
  ...
  const tabNav = ref<any>([
    { tableName: 'xgsResumeBase', tableTxt: '1. 填写基本信息' },
    { tableName: 'xgsResumeEdus', tableTxt: '2. 填写教育经历' },
    { tableName: 'xgsResumeHome', tableTxt: '3. 填写家庭状况' },
    { tableName: 'xgsResumeWorks', tableTxt: '4. 填写工作经历' },
  ]);
  const activeKey = ref('xgsResumeBase');
  const xgsResumeEdus = ref();
  const xgsResumeHome = ref();
  const xgsResumeWorks = ref();
  const tableRefs = { xgsResumeEdus, xgsResumeHome, xgsResumeWorks };
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    const jobDetail = toRef(data.jobDetail);
    if (unref(isUpdate)) {
      //表单赋值
  ...
  function classifyIntoFormData(allValues) {
    ...
    return {
      ...main, // 展开
      xgsResumeEdusList: allValues.tablesValue[0].tableData,
      xgsResumeHomeList: allValues.tablesValue[1].tableData,
      xgsResumeWorksList: allValues.tablesValue[2].tableData,
    };
  }
  //表单提交事件
...
<style lang="less">
  // Online表单Tab风格专属样式
  .xgsResumeBaseModal .ant-upload-list {
    width: 600px;
  }
  .j-cgform-tab-modal {
    .contentArea {
