...
public class VueFilesToDoc {
    ...
    public static void mergeJavaFilesToDoc(String sourceDirPath, String outputDocPath) {
        ...
        List<File> javaFiles = FileUtil.loopFiles(sourceDirPath, pathname -> {
            if (pathname.getName().toLowerCase().contains("xgs") && (pathname.getName().endsWith(".vue") || pathname.getName().endsWith(".ts"))) {
        ...
        if (javaFiles.isEmpty()) {
            System.out.println("在目录 " + sourceDirPath + " 中未找到任何.vue");
            return;
        }
        ...
        try (FileOutputStream out = new FileOutputStream(outputDocPath)) {
            document.write(out);
            System.out.println("成功将 " + javaFiles.size() + " 个.vue文件合并到: " + outputDocPath);
        } catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("前端代码：");
        String sourceDir = "D:\\zktw\\xgszp\\RMS\\jeecgboot-vue3\\src\\views";
        String outputDoc = "D:\\zktw\\xgszp\\前端代码2.doc";
