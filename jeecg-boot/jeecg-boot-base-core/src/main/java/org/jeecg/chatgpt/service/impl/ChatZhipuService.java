 ...
 */
@Slf4j
public class ChatZhipuService implements AiChatService {
    ...
    private ChatZhipuService() {
    }

    public ChatZhipuService(ClientV4 openAiClient) {
        client = openAiClient;
    }
...
//      System.out.println("model output:" + mapper.writeValueAsString(invokeModelApiResp));
        return JSONObject.toJSONString(invokeModelApiResp);
    }
    ...
    public String multiCompletions(List<MultiChatMessage> messages) {
        ...
        for (MultiChatMessage message : messages) {
            ChatMessage chatMessage = new ChatMessage(ChatMessageRole.USER.value(), message);
