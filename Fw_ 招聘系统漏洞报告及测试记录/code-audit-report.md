...
| 凭据类型 | 值 | 影响 |
|----------|-----|------|
| **数据库密码** | `xgsrms@zktw` | 直连 `111.198.53.163:18333` 的生产数据库 |
| **邮件服务密码** | `K9WW48aP!R69qBfj` | `liuyanliang@iie.ac.cn` 的 SMTP 密码（`mail.cstnet.ac.cn:587`） |
...
**注意：** 外部数据库地址 `111.198.53.163:18333` 加上凭据 `xgsrms/xgsrms@zktw` 可直接连接到生产数据库，无需通过应用层。请立即核查该数据库是否真正对外暴露（黑盒扫描中仅发现 `3306` 可达，该端口为 `18333`）。
...
```bash
mysql -h 111.198.53.163 -P 18333 -u xgsrms -p'xgsrms@zktw' xgsrms
```
